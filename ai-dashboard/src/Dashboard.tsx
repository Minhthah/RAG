...
const Dashboard: React.FC = () => {
  ...
  const handleSendMessage = (e?: React.FormEvent) => {
    ...
    // DLP Masking (Che thông tin thẻ tín dụng/CCCD)
    const dlpRegex = /\b\d{9,16}\b/g;
    const hasSensitiveData = dlpRegex.test(userText);
    if (hasSensitiveData) {
      userText = userText.replace(dlpRegex, '***[DLP_MASKED]***');
    }

    setMessages(prev => [...prev, { id: Date.now(), sender: 'user', text: userText, isMasked: hasSensitiveData }]);
    ...
    setTimeout(() => {
      ...
      if (chatMode === 'ai') {
        if (hasSensitiveData) {
          newMsg = { id: Date.now(), sender: 'ai', text: lang === 'vi' ? `[HỆ THỐNG BẢO VỆ DLP] Bộ lọc tự động đã phát hiện chuỗi dữ liệu nhạy cảm (CCCD/Thẻ tín dụng) và tiến hành che dấu trước khi gửi lên máy chủ AI.` : `[DLP PROTECTION] Sensitive data detected and masked securely.` };
        } else if (userText.toLowerCase().includes('lương') || userText.toLowerCase().includes('salary')) {
          newMsg = { id: Date.now(), sender: 'ai', text: lang === 'vi' ? `[SECURITY] Rất tiếc, bạn không có quyền truy cập thông tin này do tài liệu được gắn nhãn Tuyệt Mật.` : `[SECURITY] Access denied. Document labeled Top Secret.` };
        } else {
          newMsg = { id: Date.now(), sender: 'ai', text: lang === 'vi' ? `Dữ liệu trong hệ thống yêu cầu chữ ký Giám đốc trước khi duyệt.` : `The system data requires Director's signature.`, sources: ['Quy_trinh.docx (Trang 3)'] };
        }
      } else {
        newMsg = { id: Date.now(), sender: 'admin', text: lang === 'vi' ? `Chào bạn, mình đang kiểm tra vấn đề: "${userText}"...` : `Hi, checking the issue: "${userText}"...` };
      }
        ...
        return (
          <div className="animate-in fade-in duration-300 space-y-6">
            <div className={`${cardClass} rounded-xl p-8`}>
              ...
              <div className={`border-2 border-dashed rounded-2xl p-10 text-center transition-colors cursor-pointer group ${isDarkMode ? 'border-blue-500/30 bg-blue-900/10 hover:bg-blue-900/20' : 'border-blue-300 bg-blue-50/50 hover:bg-blue-50'}`}>
                ...
                <p className={`text-sm mb-5 ${textMuted}`}>{lang === 'vi' ? 'Hỗ trợ: PDF, DOCX, TXT - Tối đa 50MB' : 'Supports: PDF, DOCX, TXT - Max 50MB'}</p>
                <button className="bg-blue-600 text-white px-6 py-2.5 rounded-lg text-sm font-medium shadow-sm hover:bg-blue-700 transition-colors">
